...
class ScaleItemAnimator : SimpleItemAnimator() {
    ...
    override fun runPendingAnimations() {
        ...
        if (DEBUG) {
            Log.d(
                TAG,
                "removals: $removalsPending moves: $movesPending changes: $changesPending additions: $additionsPending"
            )
        }
    ...
    private fun animateMoveImpl(
        ...
        val moveAnimator = ObjectAnimator.ofFloat(view, View.TRANSLATION_Y, -deltaY.toFloat(), 0f)
        ...
        val recoverScaleAnimator = AnimatorSet().apply {
            playTogether(
        ...
        if (deltaY > 0) {
            if (DEBUG) {
                Log.d(TAG, "scale move ${view.tag}")
            }
            animationSet.playSequentially(scaleAnimator, moveAnimator, recoverScaleAnimator)
        } else {
            if (DEBUG) {
                Log.d(TAG, "delay move ${view.tag}")
            }
            animationSet.startDelay = scaleAnimator.duration
            animationSet.play(moveAnimator)
        }
    ...
    override fun animateChange(
        ...
        if (DEBUG) {
            Log.d(TAG, "animateChange ${oldHolder.itemView.tag}  -> ${newHolder?.itemView?.tag}")
        }
